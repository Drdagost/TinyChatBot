...
def check_native_binaries() -> Dict[str, bool]:
    ...
    if not bins['tesseract'] or not bins['pdftoppm']:
        ...
        if not bins['pdftoppm']:
            lines.append(" - pdftoppm (part of poppler) not found in PATH; required by pdf2image.")
...
class DocumentExtractor:
    ...
    def __init__(self, enable_ocr: Optional[bool] = None):
        ...
        if enable_ocr is None:
            self.enable_ocr = bool(self.native_bins.get('tesseract') and self.native_bins.get('pdftoppm'))
        else:
                ...
                for shape in doc.inline_shapes:
                    try:
                        if hasattr(shape, '_inline') and hasattr(shape._inline, 'docPr'):
                            ...
                            alt = getattr(docpr, 'descr', None) or getattr(docpr, 'title', None)
                            if alt:
                                parts.append(alt)
                    except Exception:
                        continue
