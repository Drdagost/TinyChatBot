...
class VectorStore:
    ...
    def __init__(self):
        ...
        if provider == "memory":
            ...
            raise NotImplementedError(f"Vector provider '{provider}' not implemented in this minimal refactor")
    ...
    def upsert(self, id: str, embedding: List[float], metadata: dict | None = None):
        ...
        for i, v in enumerate(self._vectors):
            if v.get("id") == id:
                self._vectors[i] = {"id": id, "embedding": embedding, "metadata": metadata or {}}
                return
    ...
    def query(self, embedding: List[float], top_k: int = 5):
        ...
        for v in self._vectors:
            score = dot(embedding, v["embedding"]) / (norm(embedding) * norm(v["embedding"]) + 1e-12)
