...
def chunk_text(text: str, chunk_size: int = 1000, overlap: int = 200):
    ...
    if tiktoken:
        ...
        enc = tiktoken.encoding_for_model(model_name) if hasattr(tiktoken, 'encoding_for_model') else tiktoken.get_encoding('cl100k_base')
...
def chunk_with_metadata(text: str, path: str, chunk_size_tokens: int = 700, overlap_tokens: int = 150):
    ...
    for p_idx, page_text in enumerate(pages, start=1):
        ...
        for para_idx, para in enumerate(paras, start=1):
            sub_chunks = chunk_text(para, chunk_size=chunk_size_tokens, overlap=overlap_tokens)
            for sc_idx, sc in enumerate(sub_chunks):
                meta = {'source': path, 'page': p_idx, 'para': para_idx, 'chunk_index': sc_idx}
...
def _build_index_if_needed(docs: List[Dict[str, Any]], vstore: VectorStore, llm: LLMClient, force: bool = False) -> None:
...
def qa(req: QARequest):
    ...
    for h in hits:
        meta = h.get('metadata', {})
        key = (meta.get('source'), meta.get('page'), meta.get('para'), meta.get('chunk_index'))
        if key in seen:
            continue
        seen.add(key)
        # include at minimum the source; keep snippet/page/para if available
        entry = {k: v for k, v in meta.items() if k in ('source', 'snippet', 'page', 'para', 'chunk_index')}
